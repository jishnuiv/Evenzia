...
class _ApproveRequestState extends State<ApproveRequest> {
  ...
  Future<void> viewEvents(BuildContext ctx) async {
    ...
    final response = await http.post(
      Uri.parse(UrlConst+'department-rep/participation-requests'),
      headers: {
  ...
  Future<void> approveEvents(id) async {
    print({'approveEvents', _token,});
    final response = await http.post(
      Uri.parse(UrlConst+'department-rep/approve-participation'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ${_token}' ,
    ...
    var jsondata = jsonDecode(response.body);
    print({'appr',jsondata['success']});

    var status = jsonDecode(response.body)["status_code"].toString();
    var data = jsonDecode(response.body);
    print({"appr events: ", data});
            ...
            if (!snapshot.hasData) {
              ...
              return ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    ...
                    return CustomCard(
                      title: snapshot.data['data'][index]['student']['name'].toString(),
                      date:snapshot.data['data'][index]['event']['created_at'].toString(),
                      // snapshot.data['data'][index]['created_at'],
                      organizerName:snapshot.data['data'][index]['event']['name'].toString(),
                      ...
                      onPressed: (){},
                      onPress:(){
                        approveEvents(snapshot.data['data'][index]['id'].toString(),);
                        setState(() {
                          viewEvents(context);
                        });
                      }, bname: 'Approve' ,
        ...
        RawMaterialButton(onPressed: onPressed
          ...
          child: Column(children: [
            ...
            Padding(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Text('Student:  ',style: TextStyle(color: Colors.white,fontSize:17,fontWeight: FontWeight.w500),),
